...
export class RoomService {
  ...
  getRoom(roomId: number): Room {
    return this.toRoom(this.getRoomOrThrow(roomId));
  }
  ...
  joinRoom(body: JoinRoomRequest, socketId: string): RoomJoinedResponse {
    ...
    const joinedPlayer = room.players.find(
      (roomPlayer) => roomPlayer.socketId === socketId,
    );
  ...
  leaveRoom(roomId: number, playerId: number): Room {
    ...
    if (
      ...
      !room.players.some((player) => player.role === PlayerRole.HOST)
    ) {
  ...
  private toRoom(room: StoredRoom): Room {
    return {
      ...
      players: room.players.map(({ id, socketId, name, role }) => ({
        id,
        socketId,
        name,
        role,
